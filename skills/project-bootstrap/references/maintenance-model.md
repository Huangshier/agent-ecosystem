...
## Update Rules
1. Treat `knowledge-hub/templates/languages/<language>/project-root|project-agent/`
   as the authoritative source for project-memory language templates.
2. Keep `assets/knowledge-hub-template/templates/languages/<language>/project-root|project-agent/`
   synchronized as the bundled runtime snapshot used by the bootstrap skill.
3. Treat English language templates as the default ordinary bootstrap source.
...
10. Keep long-lived project artifacts such as `docs/specs/_templates/` under each language's `project-root/`; bootstrap must install the full project-root tree, not only `AGENTS.md`.
