# 工作说明

项目记忆语言：简体中文。

- **Title（标题）**:
- **Slug（标识）**:
- **Status（状态）**: Draft / Active / Done / Archived
- **Owner（负责人）**:
- **Updated（更新时间）**:

## 1. Summary（摘要）
- 本次要构建、修改或调查什么？

## 2. 当前上下文
- 相关代码路径、二进制、文档、产物或观察到的行为。
- 已确认的现有实现事实。

## 3. 需求澄清
- 可选。当目标、范围、约束、验收标准或写入授权尚不清晰时使用。
- **已提出的问题**:
- **采用的默认假设**:
- **用户确认状态**:
- **待澄清事项**:

## 4. 目标
- 清晰列出完成结果。

## 5. 非目标
- 明确本工作项不覆盖的边界。

## 6. 约束
- 环境、兼容性、工具、时间、安全或接口约束。
- 范围控制：不要纳入目标之外的无关重构、清理或行为变更。

## 7. 假设
- 尚待证实但当前用于推进的假设。

## 8. 风险
- 可能失败或需要 fallback 的事项。

## 9. 方案
- 计划方向、实现轮廓或分析方法。

## 10. 验收与证据
- 如何验证结果。
- 完成时应留下什么证据或输出。
- 如果验收检查被跳过或暂不可用，必须先记录原因，再声明完成。

### Testing / Verification Evidence（测试 / 验证证据，可选）
- 当项目已有测试基础设施时，记录相关测试命令、通过/失败状态，以及适用的覆盖率或 fixture 证据。
- 使用 `.agents/commands/test-workflow.md` 和 `.agents/context/tech/testing-conventions.md` 选择项目特定测试证据。
- 若自动化测试不可用或本任务不需要，记录手工验证或跳过原因；不要因为没有测试框架就阻断无关工作。

## 11. 决策验证
- 可选。用于高风险或难以逆转的决策。
- **何时使用此节**:
  - 完整 ADR：跨模块架构变更、难以逆转的基础设施选择、长期维护者需要理解的权衡、团队分歧或高影响决策。
  - 仅轻量预演 / 可逆性评估：可逆的实现选择、遵循既有模式的隔离功能、影响范围小但有非琐碎假设的工作。
  - 跳过：琐碎改动、机械性文档更新、遵循既有明确模式且风险低的工作。
- **关键假设及其风险**:
- **预演失败**（此方案可能如何失败）:
- **可逆性**（撤销成本、fallback 方案）:
- **ADR**（架构决策记录，用于重要选择）:
  - 背景:
  - 决策:
  - 影响:

## 12. 循环契约
- 只在需要重复执行直到变量或条件满足时使用。
- **Variable（变量）**:
- **Source of truth（事实来源）**:
- **Check command（检查命令）**:
- **Pass predicate（通过条件）**:
- **Iteration action（迭代动作）**:
- **State record（状态记录）**:
- **Limits（限制）**:
- **Abort conditions（中止条件）**:

## 13. 执行契约
- 多阶段工作需要 agent 在每个已验证阶段后继续时使用。
- **Autonomy level（自主级别）**: ask-before-each-phase / autonomous-until-blocked / bounded-autonomous
- **Phase list（阶段列表）**:
  - P01:
  - P02:
  - P03:
- **Continue rule（继续规则）**:
- **Stop rule（停止规则）**: 包含 scope drift、unrelated refactor、skipped acceptance checks、安全/权限阻塞和无法消解的真实歧义。
- **State record（状态记录）**:

## 14. 开放问题
- 仍可能阻塞或改变执行方向的问题。
