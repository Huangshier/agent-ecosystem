...
## 全局经验发现
全局经验是可选、按需使用的查找层，只用于 toolchain、host、shell、build、
cache、port、permission、path handling、environment variables 等可跨项目复用、
反复出现的工作流问题。不要预加载全局经验，也不要假设每个 host 都存在索引。

适用时按以下顺序查找：

1. 若配置的全局经验索引存在，读取该索引。
2. 优先用已观察到的错误文本、工具名和症状匹配条目的 `keywords`，再匹配 `title`。
3. 只读取命中的条目；仅在符合当前上下文时应用已记录的 prevention rule。
4. 索引缺失或无匹配时 fail-soft，继续项目本地诊断。

明显依赖当前仓库业务逻辑、硬件接线、协议实现或模块设计的问题，默认不触发全局检索。

项目经验默认保留在项目内。经复核的跨项目候选继续进入既有 intake / triage；
禁止自动 promotion。
