...
- Made `project-bootstrap` fail closed when a project has existing memory but
  ...
  behavior. (#369)
- Preserved hidden legacy template files across Ubuntu and macOS by aligning
  recursive template enumeration and template-tree hash checks; explicit
  `-LegacyWorkspace` now copies `.claude/settings.json` without causing
  `hub.lock` drift. (#372)
