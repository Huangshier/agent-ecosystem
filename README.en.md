...
Current release: `v0.7.1` (latest published release)
...
- `optional-refresh`: the template baseline drifted or scaffold files are
  ...
  templates.
- `migration-required`: the project workspace is legacy or a structural memory
  finding was detected. Run `scripts/migrate-project.ps1 -Mode Analyze`
  (read-only), review the evidence, then explicitly `Apply`, and use the guarded
  `Rollback` if needed; do not use retired memory helpers as the migration
  authority.
