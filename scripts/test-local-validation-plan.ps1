...
function Assert-PlanCase {
    ...
    if ([int]$value.local_plan.schema_version -ne 1) { throw "$Name has no schema-1 local plan." }
    & $planContract -Result $value | Out-Null
    $iterationScripts = @($value.local_plan.stages.iteration.actions.script)
    if ($iterationScripts -contains "scripts/validate-release.ps1" -or $iterationScripts -contains "scripts/test-heavy-targeted-regression.ps1") {
        throw "$Name iteration plan includes full or heavy validation."
    }
...
if (@($prePushDryRun.actions | Where-Object suite -eq "heavy-targeted-regression").Count -ne 2) { throw "Control-surface pre-push dry-run did not plan dual-host heavy regression." }
if (@($prePushDryRun.actions | Where-Object suite -eq "full-release-validation").Count -ne 2) { throw "Tier 3 pre-push dry-run did not plan dual-host full validation." }
