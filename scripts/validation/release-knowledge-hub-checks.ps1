...
try {
    ...
    if ($resultCount -lt 1) {
        ...
        foreach ($case in $exactSearchCases) {
            ...
            if (@($exactSearch.results).Count -lt 1) {
                throw ("Experience search for '{0}' returned no matching entries." -f $case["query"])
            }
            if ([string]$exactSearch.results[0].title -ne [string]$case["expected"]) {
                throw ("Unexpected search result for '{0}': {1}" -f $case["query"], [string]$exactSearch.results[0].title)
            }
        }
        ...
        foreach ($case in $rankingCases) {
            ...
            for ($i = 0; $i -lt 2; $i++) {
                if ([string]$rankedResults[$i].title -ne [string]$case["expected_titles"][$i]) {
                    throw ("Unexpected result order for '{0}' at position {1}: {2}" -f $case["query"], $i, [string]$rankedResults[$i].title)
                }
                if ([int]$rankedResults[$i].score -ne [int]$case["expected_scores"][$i]) {
                    throw ("Unexpected score for '{0}' at position {1}: {2}" -f $case["query"], $i, [int]$rankedResults[$i].score)
                }
            }
        }
