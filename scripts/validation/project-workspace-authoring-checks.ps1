...
try {
    ...
    Add-Case -Name "promote-analyze-is-read-only" -Passed $analyzeReadOnly -Detail "Analyze emits a repository-relative Skill candidate and boundary changes without project writes."

    $applied = Invoke-WorkspaceJson -OperationParameters ([ordered]@{ Operation = "promote-skill"; ProjectRoot = $runRoot; Id = "slice-d-procedure"; Apply = $true; Json = $true })
    ...
    Add-Case -Name "promote-apply-removes-procedure" -Passed $applyAuthority -Detail "Explicit Apply creates the Skill and removes the original Procedure without dual authority."

    $parsedAfterApply = Invoke-ParserJson -Root $runRoot
    $skillAssetCount = @($parsedAfterApply.assets | Where-Object { [string]$_.type -ceq "skill" -and [string]$_.id -ceq "slice-d-procedure" }).Count
    $parserReadsSkill = ([string]$parsedAfterApply.status -ceq "PASS" -and $skillAssetCount -eq 1)
    Add-Case -Name "parser-reads-promoted-skill" -Passed $parserReadsSkill -Detail "Existing parser reads the promoted project-local SKILL.md."
    ...
    $summary = [ordered]@{
        ...
        parser_discover_reads_new_assets = $parserReadsCreated -and $parserReadsSkill -and $discoverCheckSkill
        cases = @($cases.ToArray())
    }
}
catch {
