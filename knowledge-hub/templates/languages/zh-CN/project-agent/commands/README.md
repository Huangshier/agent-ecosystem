...
- `test-workflow.md`：记录项目自身的测试框架、命令、CI 入口和证据要求。
  使用项目已有命令；不要仅凭本模板替项目选择新的测试框架。
- `skills.md`：从已有 `SKILL.md` frontmatter 和已安装位置发现可用 skills。
  只读；不要仅凭 metadata 形状安装、启用、验证或声称客户端兼容性。
